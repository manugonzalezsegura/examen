...
def guardar_nombre_medicamento(request):
    if request.method == 'POST':
        ...
        nombres_medicamentos = data.get('nombre_medicamento')
        
        if nombres_medicamentos:
            for nombre in nombres_medicamentos:
                nueva_carta = Carta(nombre=nombre)
                nueva_carta.save()
            return JsonResponse({'success': True, 'message': 'Nombres de medicamentos guardados exitosamente.'})
        else:
            return JsonResponse({'success': False, 'message': 'Ningún nombre de medicamento proporcionado.'})
